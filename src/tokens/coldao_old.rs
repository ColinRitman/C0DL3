...
pub const CD_TIER_INTEREST: [u64; NUM_TIERS] = [
    ...
    2_160_000,      // Tier 1: 0.8 XFG × 12mo @ 27%
    14_400_000,     // Tier 2: 8   XFG × 3mo  @ 18%
    26_400_000,     // Tier 3: 8   XFG × 12mo @ 33%
    216_000_000,    // Tier 4: 80  XFG × 3mo  @ 27%
    336_000_000,    // Tier 5: 80  XFG × 12mo @ 42%
    2_640_000_000,  // Tier 6: 800 XFG × 3mo  @ 33%
...
mod tests {
    ...
    fn test_cd_interest_exact_values() {
        // Verify constants match COLD_TIER_REFERENCE.md
        assert_eq!(CD_TIER_INTEREST[0], 640_000);        // Tier 0: 0.8 XFG × 3mo @ 8%
        assert_eq!(CD_TIER_INTEREST[1], 2_160_000);      // Tier 1: 0.8 XFG × 12mo @ 27%
        assert_eq!(CD_TIER_INTEREST[2], 14_400_000);     // Tier 2: 8 XFG × 3mo @ 18%
        assert_eq!(CD_TIER_INTEREST[3], 26_400_000);     // Tier 3: 8 XFG × 12mo @ 33%
        assert_eq!(CD_TIER_INTEREST[4], 216_000_000);    // Tier 4: 80 XFG × 3mo @ 27%
        assert_eq!(CD_TIER_INTEREST[5], 336_000_000);    // Tier 5: 80 XFG × 12mo @ 42%
        assert_eq!(CD_TIER_INTEREST[6], 2_640_000_000);  // Tier 6: 800 XFG × 3mo @ 33%
    ...
    fn test_ledger_multi_tier_balance() {
        let mut ledger = ColdaoLedger::new();
        ledger.mint("0xalice_addr_000000000000000000", 5, 336_000_000);
        ledger.mint("0xalice_addr_000000000000000000", 7, 5_520_000_000);
        let total = ledger.total_balance_of("0xalice_addr_000000000000000000");
        assert_eq!(total, 336_000_000 + 5_520_000_000);
    }
    ...
    fn test_governance_weight() {
        ...
        assert_eq!(weight, 336_000_000 + 5_520_000_000);
    }
}
