    ...
    ReentrancyGuard 
{
    ...
    /**
     * @dev Mint a genesis 𝝣lderado NFT by staking 80B HEAT tokens
     * @param to The address to mint to
     */
    function mintElderado(address to) external payable nonReentrant {
        require(mintingEnabled, "Minting not enabled");
        require(_tokenIdCounter.current() < MAX_SUPPLY, "Max supply reached");
        require(to != address(0), "Invalid recipient");
        require(msg.value == STAKING_AMOUNT, "Must stake exactly 80B HEAT tokens to mint");

        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();

        // Generate validator data automatically
        string memory validatorName = string(abi.encodePacked("𝝣lderado #", Strings.toString(tokenId + 1)));
        string memory validatorType = "Genesis Validator";
        uint256 powerLevel = 100;
        uint256 stakeMultiplier = 3;
        string memory specialAbility = "Genesis Blessing";
        uint256 rarityScore = 1000;
    ...
    /**
     * @dev Mint a genesis 𝝣lderado NFT to the caller by staking 80B HEAT tokens
     */
    function mintElderadoToSelf() external payable {
        mintElderado(msg.sender);
    }
